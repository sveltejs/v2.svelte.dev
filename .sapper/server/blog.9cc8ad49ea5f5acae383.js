...
Index.filename = "/Users/208311/Development/SVELTE/svelte.technology/routes/blog/index.html";
...
${__WEBPACK_IMPORTED_MODULE_0__components_Layout_html__["a" /* default */]._render(__result, {page: "blog"}, { store: options.store, slotted: { default: () => `
	<div class="posts" svelte-4227977640>
		${ state.posts.map(post => `<article class="post" svelte-4227977640>
				<a href="/blog/${post.slug}">
					<h2 svelte-4227977640>${__escape(post.metadata.title)}</h2>
					<p class="standfirst" svelte-4227977640>${__escape(post.metadata.description)}</p>
				</a>

				<p class="byline" svelte-4227977640>
					<a href="${post.metadata.authorURL}">${__escape(post.metadata.author)}</a>
					<time datetime="${post.metadata.pubdate}" svelte-4227977640>${__escape(post.metadata.dateString)}</time>
